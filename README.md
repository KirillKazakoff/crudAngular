...
### **Обработка ошибок** 
- При потере соединения и попытке отправить запрос на сервер всплывет окно с описанием ошибки.
...
- Форма создания новых записей и записи в таблице отличаются по 2 параметрам:
...
### **Глобальные стили**
- Использование стилей глобально было ошибкой, так как в Angular шаблон оборачивается в отдельный тэг, который можно стилизовать только при использовании псевдокласса :root, которым, в свою очередь, можно воспользоваться только из отдельного файла, подключенного напрямую к компоненту (либо в инлайновых стилях).

</br>

  <details>
  <summary>🦔</summary>
    Вы развернули ёжика и дочитали до конца 👍
  </details>
...
npm run start
```
Затем, запустите json-server, на котором будут храниться данные (Данные лежат в db.json)
```sh
npm run server
```
